...
/*
 * Copyright 2016 Chris Baumbauer <cab@cabnetworks.net>
 */
...
func (d *Driver) getIPfromVmnetConfiguration(macaddr string) (string, error) {
    ...
    switch runtime.GOOS {
    case "darwin":
        dhcpPath = "/library/Preferences/VMware Fusion/vmnet*/dhcp.conf"
    case "linux":
        dhcpPath = "/etc/vmware/vmnet*/dhcp/dhcpd.conf"
    }

	confFiles, _ := filepath.Glob(dhcpPath)
...
func (d *Driver) getIPfromDHCPLease(macaddr string) (string, error) {
    ...
    switch runtime.GOOS {
    case "darwin":
        leasePath = "/var/db/vmware/*.leases"
    case "linux":
        leasePath = "/etc/vmware/vmnet*/dhcpd/dhcpd.leases"
    }

	leasesFiles, _ := filepath.Glob(leasePath)
